...
csv_namefile2 = 'Samples-AYA.csv'#le fichier excel des classes utilisées pour former le modèle
csv_namefile = 'Test-6-JV&GR-Presets.csv' #le fichier excel de wavfile de test 
    ...
    for fn in tqdm(os.listdir(wavfile_namedir)):#loop sur les filennames des morceaux de wavfile de test nettoyé
        ...
        for i in range(0,wav.shape[0]-config.step, config.step):#loop sur la longueur de wavfile avec un pas de 1/10s
            ...
            if mask.count(True) == len(mask):#si True donc l'échantillon est supérieur au seuil
                ...
                cl[t]= mfcc(sample,16000,numcep=config.nfeat,nfilt=config.nfilt,
                       nfft = config.nfft,winlen=0.032,winstep =0.033
                      )#préparation de l'échantillon en utilisant la formule mfccs
            t +=100    
